...
def train_model():
    ...
    # train_thread.join()
    # test_thread.join()
    feature_df_train = extracTrainFea(train_df, sampleRate=0.5)
    feature_df_train.to_csv("train_feature.csv", index=True)
    feature_df_test = extracTestFea(test_df)
    feature_df_test.to_csv("test_feature.csv", index=True)
    ...
    total_tf = norm_feature(total_df, feature_name)
